...
```{r}
  ...
  risk_factors <- c('age', 'raceth', 'BMI', 'smoking', 'drinkany', 'exercise', 'statins')
  names(outcome_vars) <- outcome_vars
  lms.adjusted <- lapply(outcome_vars, function(outcome) {
...
```{r}
  ...
  lm1 <- lm(LDL.diff ~ treatment + statins + age + raceth + BMI + smoking + drinkany + exercise , data=clean_hers)
  anova.LDL <- anova(lm0, lm1)
  
  lm0 <- lm(HDL.diff ~ treatment + statins, data=clean_hers)
  lm1 <- lm(HDL.diff ~ treatment + statins + age + raceth + BMI + smoking + drinkany + exercise , data=clean_hers)
  anova.HDL <- anova(lm0, lm1)
  
  lm0 <- lm(TG.diff ~ treatment, data=clean_hers)
  lm1 <- lm(TG.diff ~ treatment + statins + age + raceth + BMI + smoking + drinkany + exercise , data=clean_hers)
